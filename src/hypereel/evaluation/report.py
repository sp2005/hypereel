...
def write_report(report: dict, output_dir: str | Path) -> Path:
    ...
    lines = [f"# HypeReel {mode} evaluation", "",
             ...
             f"Degraded: {report.get('degraded_count', 0)} (excluded from aggregates).", "",
    ...
    for name, group in data["aggregates"].items():
        ...
        lines.append("")
    lines += ["## Cases", "", "| Case | Status | Clips | Duration | Elapsed seconds | Error |",
              "|---|---|---:|---:|---:|---|"]
    for case in report["cases"]:
        m = case["metrics"]
        lines.append(f"| {_cell(case['case_id'])} | {case['status']} | {m.get('clip_count', 'N/A')} | "
                     f"{m.get('selected_duration_seconds', 'N/A')} | {case['elapsed_seconds']:.6f} | "
